...
Example config:
    ...
    # Workflow-level overrides: enable/disable experiments for specific workflows.
...
def get_runner_prefix(
    ...
    for experiment_name, experiment_settings in settings.experiments.items():
        ...
        # before global rollout. Only applies when no user config matched.
            ...
            wf_optout = any(
                c.name == f"-{experiment_name}" for c in wf_configs
            )
            if wf_optout:
                log.info(
                ...
                continue
            wf_optin = next(
                (c for c in wf_configs if c.name == experiment_name), None
            )
            if wf_optin:
                if wf_optin.rollout_perc >= 100 or random.uniform(0, 100) <= wf_optin.rollout_perc:
                    log.info(
